...
class FeRe_Stripe(nn.Module):
    def __init__(
        ...
        """
        FeRe_Stride: Modified version of FeRe which resamples features within
        a stripe of features across the time-axis
        ...
        Feature Resampling, randomly samples a uniform distribution $U[a,1]$ with
        hyperparam $a$. By sampling, it gets the `copy_area` (float), which denotes the 
        proportion (=area) of the modality that is going to be resampled
        (i.e how many features are going to be copied).
        For each sample in a batch we sample either a single or independent beta
        distributions with their own $a$ hypeparams.
        A PORTION OF EACH MODALITY IS BEING RESAMPLED FROM SOME OTHER EXISTING FEATURE VECTOR.
        Args:
            ...
            single_pick (bool): when true samples from a Betta distribution for each one
                of the involved modalities, otherwise from a uniform
        """
        super(FeRe_Stripe, self).__init__()
        ...
        if self.training:
            ...
            if len(self.beta_mod) == 1 or self.single_pick:
                ...
                for i, _ in enumerate(self.beta_mod):
                    copy_area.append(self.beta_mod[i].sample((bsz, )))
            ...
            for i_modal, p_modal in enumerate(self.p_t_mod):
                ...
                if self.mask_dim:
                    ...
                    # area_mask = \
                    #     torch.ones((bsz, seqlen, d_modal)).to(mods[0].device)
                    area_mask = \
                        torch.ones((bsz, d_modal)).to(mods[0].device)
                    stripe_len = \
                        (copy_area[i_modal] * d_modal).type(torch.int).to(mods[0].device)
                    max_stripe_start = d_modal - stripe_len
                    for i in range(bsz):
                        stripe_start = \
                            random.randint(0, max_stripe_start[i].item())
                        stripe_offset = stripe_len[i].item()
                        area_mask[i, stripe_start:stripe_start+stripe_offset] = 0.0
                    area_mask = area_mask.unsqueeze(1)
                    # import pdb; pdb.set_trace()
                else:
                    # general case
