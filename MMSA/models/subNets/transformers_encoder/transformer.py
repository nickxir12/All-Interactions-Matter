...
def buffered_future_mask(tensor, tensor2=None):
    ...
    if tensor2 is not None:
        dim2 = tensor2.size(0)
    future_mask = torch.ones(dim1, dim2).to(tensor.device)
    future_mask = fill_with_neg_inf(future_mask)
    future_mask = torch.triu(future_mask, 1+abs(dim2-dim1))
    return future_mask[:dim1, :dim2]


def buffered_future_mask_old(tensor, tensor2=None):
    dim1 = dim2 = tensor.size(0)
    if tensor2 is not None:
        dim2 = tensor2.size(0)
    future_mask = \
        torch.triu(fill_with_neg_inf(torch.ones(dim1, dim2)), 1+abs(dim2-dim1))
    if tensor.is_cuda:
