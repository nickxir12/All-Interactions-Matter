...
class AlignSubNet(nn.Module):
    ...
    def __ctc(self, text_x, audio_x, video_x, real_len=None):
    ...
    def __avg_pool(self, text_x, audio_x, video_x, real_len=None):
        def align(x, real_len=None):
            raw_seq_len = x.size(1)
            if raw_seq_len == self.dst_len:
                return x
            # check for real len argument
        ...
        if isinstance(audio_x, tuple):
            tup_x = audio_x[1]
            audio_x = align(audio_x[0], real_len[0] if real_len else None)
            audio_x = (audio_x, tup_x)
        else:
            audio_x = align(audio_x, real_len[0] if real_len else None)

        if isinstance(video_x, tuple):
            tup_x = video_x[1]
            video_x = align(video_x[0], real_len[0] if real_len else None)
            video_x = (video_x, tup_x)
        else:
            video_x = align(video_x, real_len[0] if real_len else None)
        
        # video_x = align(video_x, real_len[1] if real_len else None)
        return text_x, audio_x, video_x
    
    def __conv1d(self, text_x, audio_x, video_x, real_len=None):
    ...
    def forward(self, text_x, audio_x, video_x, real_len=None):
        # already aligned
        ###########################################################################################
        ## TETFN CASE ONLY
        if isinstance(audio_x, tuple):
            if text_x.size(1) == audio_x[0].size(1) == video_x[0].size(1):
                return text_x, audio_x, video_x
            return self.ALIGN_WAY[self.mode](text_x, audio_x, video_x, real_len=real_len)
        ###########################################################################################
        else:
