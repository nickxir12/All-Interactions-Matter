...
class TAFC_O5():
                ...
                for batch_data in td:
                    ...
                    if self.n_bn_fusion > 0 and self.modded_loss:
                        ...
                        if self.use_ulgm and epochs > self.ulgm_patience:
                            for m in self.tasks:
                                if m == 'bn_logits':
                                    y_pred[self.name_map[m]].append(
                                        torch.mean(outputs[m], dim=1).to(torch.float32).cpu().detach()
                                    )
                                else:
                                    y_pred[self.name_map[m]].append(
                                        outputs[m].to(torch.float32).cpu().detach()
                                    )
                                y_true[self.name_map[m]].append(
                                    self.label_map[self.name_map[m]][indexes].to(torch.float32).cpu().detach()
                                )
                            if self.use_bf16:
                                with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                                    for m in self.tasks:
                            ...
                            if self.use_cmc_loss:
                                if self.use_bf16:
                                    with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                                        cmc_loss = self.crit_cmc(av_logits, bn_logits)
                    ...
                    if (self.n_bn_fusion > 0) and self.modded_loss:
                        # modded loss
                        # av distil loss - cross-modal consistency loss
                        # if self.use_bf16:
                        #     with torch.autocast(device_type='cuda', dtype=torch.bfloat16): 
                        #         av_distil = \
                        #             self.distil_av_crit_xL(task_logits, av_logits)
                        #         loss += av_distil * self.w_av_distil
                        pass
                    else:
                        # av distil loss
                        if self.use_bf16:
                    ...
                    if self.use_clm_loss:
                        ...
                        if self.use_bf16:
                            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                                ...
                                # print(f"Updating clm loss")
                                w_gamma = self.gamma
                                # import pdb; pdb.set_trace()
                                # print(f"ongoing with {w_gamma}")
                                # total_loss
    ...
    def do_test(self, model, dataloader, mode="VAL", return_sample_results=False):
            ...
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    if self.use_ulgm:
                        ...
                        #     if m == 'bn_logits':
                        #         y_pred[self.name_map[m]].append(
                        #             torch.mean(outputs[m], dim=1).to(torch.float).cpu().detach()
                        #         )
                        #     else:
                        #         y_pred[self.name_map[m]].append(
                        #             outputs[m].to(torch.float).cpu().detach()
                        #         )
                        #     # y_pred[self.name_map[m]].append(outputs[m].to(torch.float).cpu())
                        #     y_true[self.name_map[m]].append(
                        #         self.label_map[self.name_map[m]][indexes].to(torch.float).cpu().detach()
                        #     )
        ...
        if self.use_ulgm:
            ...
            #     pred, true = \
            #         torch.cat(y_pred[self.name_map[m]]), torch.cat(y_true[self.name_map[m]])
            #     _eval_results = self.metrics(pred, true)
            #     logger.info('%s: >> ' %(m) + dict_to_str(_eval_results))
            #     if m == 'task_logits':
